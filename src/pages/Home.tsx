...
export default function Home() {
  ...
  return (
    <div className="space-y-6 p-3 lg:p-5 max-w-7xl mx-auto">
      {/* Hero */}
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-accent/5 to-background border border-border px-5 py-10 lg:px-8 lg:py-16 text-center">
      ...
      {/* Features */}
      <section>
        <h2 className="text-xl font-display font-bold text-foreground mb-5">Platform Capabilities</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((f) => (
            <div key={f.title} className="p-5 rounded-xl bg-card border border-border hover:shadow-md transition-shadow">
      ...
      {/* Quick Links */}
      <section>
        <h2 className="text-xl font-display font-bold text-foreground mb-5">Quick Access</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickLinks.map((link) => (
            <Link
              key={link.path}
